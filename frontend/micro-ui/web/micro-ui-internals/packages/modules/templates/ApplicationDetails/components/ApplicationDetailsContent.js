                ...
                detail?.values?.map((value, index) => {
                  ...
                  if (value?.isLink == true)
                  {
                    return (
                      <Row
                        key={t(value.title)}
                        label={window.location.href.includes("tl") ? <div style={{width:"200%"}}><Link to={value?.to}><span className="link" style={{color: "#1F4A7C"}}>{t(value?.title)}</span></Link></div> : isNocLocation || isBPALocation ? `${t(value.title)}` : t(value.title)}
                        text={<div><Link to={value?.to}><span className="link" style={{color: "#1F4A7C"}}>{value?.value}</span></Link></div>}
