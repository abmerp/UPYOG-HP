  ...
  const getCheckBoxLable = () => {
    return (
      <div>
        ...
        <span style={{color: "#1F4A7C", cursor: "pointer"}} onClick={() => setShowTermsModal(!showTermsModal)}>{t(`BPA_TERMS_AND_CONDITIONS_LABEL`)}</span>
      </div>
    )
  }
            ...
            {!detail?.isNotAllowed ? <Card key={index} style={!detail?.additionalDetails?.fiReport && detail?.title === "" ? { marginTop: "-30px" } : {}}>
              ...
              <StatusTable>
                ...
                {(detail?.isCommon && detail?.values?.length > 0) ? detail?.values?.map((value) => {
                  ...
                  if (value?.isLink) return <Row className="border-none" label={t(value?.title)} text={<div><Link to={value?.to}><span className="link" style={{color: "#1F4A7C"}}>{value?.value}</span></Link></div>} />
