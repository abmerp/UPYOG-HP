import React from "react";
import "../styles/certificates.css";
import barImg from "../assets/bar.png";
import logo from "../assets/logo.png";
import sign from "../assets/sign.png";
import swatchBharat from "../assets/swatchBharat.png";
import watermark from "../assets/watermark.png"; 
import Regi2TCPSign from "../assets/Regi2TCPSign.png";
import QrCode from "./QRCODE";

const TCPRegi2Cert = () => (
  <div className="certificate-container">
    <div className="certificate">
      <div className="water-mark-overlay"></div>
      <div class="outer-border"></div>
      <div class="inner-border"></div>
      <div className="certificate-header">
      <img
          src={logo}
          style={{height:147,width:145}}
          alt=""
        />
       <p className="certificate-title">
          <strong>
            Department of Town and Country Planning <br /> Government of
            Himachal Pradesh
          </strong>
         
          {/* <h1 style={{fontSize:30}}>Mobile Tower Application</h1> */}
          <h1 className="border-title" style={{width:"50%",marginLeft:"170px"}}>CERTIFICATE OF REGISTRATION</h1>
        <br/>
        <div>
        <h6 >Registration of Structural Engineers under Rule 21 read with notification no. TCP-A(3)-1/2014-III</h6> 
        </div>
      <br/>
      <h6 style={{fontWeight:400}}>Registration NO:RS0158/2020</h6>
    
         </p>
       
        <div>
          <QrCode/>
        {/* <img src="data:image/png;base64," 
        className="logo" alt="" style={{height:"60",width:"60"}}/> */}
         <h2 className="srNo">Sr. No. : 0000</h2>
        </div>
      </div>

      <div className="certificate-body" >
      
        
{/* 
        <div className="certificate-date">
       
          <h2>No. : RP2074/2022</h2>
        </div> */}
        {/* <div className="certificate-to">
          <h6>To</h6>
        </div>
        <div className="name">
          Sh./Smt./M/s
          <span className="applied-name">
          Mr. GOKUL BUTAIL&nbsp;&nbsp;&nbsp;&nbsp;<span style={{fontWeight:"400"}}>C/o:</span>S/o DINESH BUTAIL
          OPPOSITE FIRE STATION PALAMPUR, , PALAMPUR, Palampur, Kangra, Himachal Pradesh
          </span>
        </div>
        <div className="certificate-to">
          <h6>
            Subject :-Permission for Change of Land Use..
          </h6>
        </div> */}
        {/* <div className="certificate-to">
          <h6>
            Reference :- Your application No. 01202000270 .Dated 15-Jul-2020 
          </h6>
        </div> */}
        <div className="body">
          <p>
       <span style={{fontWeight:500}}> Mr. Abhishake Kumar, S/o Balak Ram, Vill. Choundha P.o. Doulthpur Distt. Kangra (h.p.), Town/Village-Vill.
Chounda P.o. Doultpur, PO/Tehsil-Kangra, District-Kangra(Himachal Pradesh) </span>is registered as  <span style={{fontWeight:500}}>Structural
Engineer</span> professional for the preparation of structural design,safety of building and to sign the plans with in the limits of
Planning Areas/Special Areas/Deemed Panning Areas under the provisions of Himachal Pradesh Town and Country
Planning Act, 1977. The Registration is subject to the Act, Rules and Regulations and is valid upto <span style={{fontWeight:500}}>23-Jul-2025</span> The
structural Engineer will be responsible for the safety and stability of the structure. In case of any violations or dereliction
in preparing of building map, structural design and analysis, registration is likely to canceled.
</p>
        </div>
        {/* <div className="certificate-signature">
          <img src={TCPForm11Sign} className="logo" alt="" />
        </div> */}
        {/* <div className="certificate-to">
          <h6>Copy To:-</h6>
        </div>
        <div className="list">
          <h5>
            1. The development/ sub-division of land/ construction of building shall be carried out strictly in accordance with the
                approved Change of Land Use.
          </h5>
          <h5>
            2. . In future, if any sub-division is involved, permission for the same shall have to be taken separately as per provisions
                of Himachal Pradesh Town and Country Planning Act, 1977
          </h5>
          <h5>
            3. Planning permission shall have to be taken separately
            4.Applicant have to take permission from municipal corporation for development under section 18.5.2 of Development
              Plan Palampur
          </h5>
        </div> */}
        <div className="certificate-date">
       
       <h2>Place. :  Shimla</h2>
       <h2>Date : 25-Nov-2022</h2>
     </div>
        <div className="certificate-to">
          <h6> Enclosers : As above :-</h6>
        </div>
        <div className="certificate-last">
          <img src={swatchBharat} alt="" />
          <img src={Regi2TCPSign} alt="" className="sign" />
        </div>
      </div>
    </div>
  </div>
);

export default TCPRegi2Cert;
